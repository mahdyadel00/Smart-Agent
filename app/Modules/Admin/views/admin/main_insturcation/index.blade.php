...
[
	'title' => _i('Main Insturcation'),
	'subtitle' => _i('Main Insturcation'),
	'activePageName' => _i('Main Insturcation'),
	'activePageUrl' => route('main_insturcation.index'),
	'additionalPageName' =>  _i('Main Insturcation'),
	'additionalPageUrl' => route('main_insturcation.index')
])
@section('content')
<div class="box-body">
	<div class="row">
        @can('Create_main_insturcation')
		<div class="col-sm-12 mb-3">
			<span class="pull-left">
			<button class="btn btn-primary create add-permissiont" type="button" data-toggle="modal"
				data-target="#modal-default">
				<span><i class="ti-plus"></i> {{_i('Create new Main Insturcation')}} </span>
			</button>
			</span>
		</div>
        @endcan
...
@include('admin.main_insturcation.modal')
@endsection
@push('js')
<script>
	var table = $('.dataTable').DataTable({
		...
		processing: true,
		serverSide: true,
		ajax: '{{route('main_insturcation.index')}}',
		columns: [
			{data: 'id', name: 'id'},
	...
	{
		e.preventDefault();
		let url = "{{ route('main_insturcation.update') }}";
		$.ajax({
			url: url,
			method: "post",
			data: new FormData(this),
    ...
    //    alert(transRowId);
		$.ajax({
			url: '{{route('main_insturcation.get.translation')}}',
			method: "get",
			"_token": "{{ csrf_token() }}",
			data: {
				'lang_id': lang_id,
