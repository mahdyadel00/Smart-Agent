...
[
	'title' => _i('GoalsModules'),
	'subtitle' => _i('GoalsModules'),
	'activePageName' => _i('GoalsModules'),
	'activePageUrl' => route('goals_modules.index'),
	...
	<div class="row">
		...
		<div class="col-sm-12 mb-3">
			...
			<button class="btn btn-primary create add-permissiont" type="button" data-toggle="modal"
				...
				<span><i class="ti-plus"></i> {{_i('Create new GoalsModules')}} </span>
			</button>
			</span>
		</div>
        @endcan
...
@include('admin.goals_modules.modal')
@endsection
@push('js')
<script>
	var table = $('.dataTable').DataTable({
		...
		processing: true,
		serverSide: true,
		ajax: '{{route('goals_modules.index')}}',
		columns: [
			{data: 'id', name: 'id'},
	...
	{
		e.preventDefault();
		let url = "{{ route('goals_modules.update') }}";
		$.ajax({
			url: url,
			method: "post",
			data: new FormData(this),
    ...
    //    alert(transRowId);
		$.ajax({
			url: '{{route('goals_modules.get.translation')}}',
			method: "get",
			"_token": "{{ csrf_token() }}",
			data: {
				'lang_id': lang_id,
